...
- **Optional TypeSafe Jev Routing**: `model: "auto"` can semantically select a currently routable model across active providers for OpenAI, Anthropic, and Gemini compatibility routes, with deterministic fail-open fallback. See [TypeSafe Jev model selection](docs/configuration.md#typesafe-jev-model-selection).
